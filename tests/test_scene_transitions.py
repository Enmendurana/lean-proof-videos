...
def test_new_rows_are_written_and_matching_stays_inside_a_block() -> None:
    ...
    with (
        ...
        patch("proof_video.animation.scene_helpers.Succession", side_effect=sequence) as succession,
    ):
...
def test_intro_binder_moves_into_its_new_context_row() -> None:
    ...
    transition = SemanticTransition(
        source=SemanticExpression((
            SemanticExpressionNode(
                "forall-binder", kind="declaration",
                latex_spans=(SemanticSpan(2, 3),),
            ),
        )),
        target=SemanticExpression((
            SemanticExpressionNode(
                "context-x", kind="declaration",
...
def test_same_lineage_continues_one_visual_block() -> None:
    ...
    continuation = TaggedGroup(
        [block("goal-0", row("hyp-new:0"), row("target:0"))]
    )
...
def test_latex_index_map_drives_token_transform_without_shape_guessing() -> None:
    ...
    with (
        ...
        patch("proof_video.animation.scene_helpers.AnimationGroup", side_effect=parallel),
    ...
    assert animations == [("sequence", ("parallel",
...
def test_introduced_tokens_arrive_oversized_while_the_row_moves_in_parallel() -> None:
    ...
    with (
        ...
        patch("proof_video.animation.scene_helpers.FadeOut", return_value="fade-out") as fade_out,
        patch("proof_video.animation.scene_helpers.FadeIn", return_value="fade-in") as fade_in,
        patch("proof_video.animation.scene_helpers.AnimationGroup", side_effect=parallel),
        patch("proof_video.animation.scene_helpers.Succession", side_effect=sequence),
    ):
        animations = _mapped_row_animations(source, target, maps)

    assert animations == [("sequence",
...
def test_contiguous_semantic_tokens_use_one_group_transform() -> None:
    ...
    assert animation == ("sequence", ("parallel",
...
def test_relations_operators_and_punctuation_require_a_logical_owner() -> None:
    ...
    transition = SemanticTransition(
        source=SemanticExpression((
            SemanticExpressionNode(
                "old-expression", kind="app", path=(0,),
                latex_spans=(SemanticSpan(0, len(tokens)),),
            ),
        )),
        target=SemanticExpression((
            SemanticExpressionNode(
                "new-expression", kind="app", path=(0,),
                latex_spans=(SemanticSpan(0, len(tokens)),),
            ),
        )),
        edges=(SemanticTransitionEdge(
...
def test_same_symbol_at_same_position_is_not_enough_without_a_lean_edge() -> None:
    ...
    transition = SemanticTransition(
        source=SemanticExpression((SemanticExpressionNode(
            "old", latex_spans=(SemanticSpan(0, 6),)
        ),)),
        target=SemanticExpression((SemanticExpressionNode(
            "new", latex_spans=(SemanticSpan(0, 6),)
        ),)),
        edges=(),
    )

    assert _supplement_logically_stable_syntax_pairs(
...
def test_index_map_never_morphs_different_latex_tokens() -> None:
    ...
    with (
        patch("proof_video.animation.scene_helpers.Transform", return_value="bad-morph") as transform,
        patch("proof_video.animation.scene_helpers.FadeOut", return_value="fade-out") as fade_out,
        patch("proof_video.animation.scene_helpers.FadeIn", return_value="fade-in") as fade_in,
        patch("proof_video.animation.scene_helpers.AnimationGroup", side_effect=parallel),
        patch("proof_video.animation.scene_helpers.Succession", side_effect=sequence),
...
def test_identical_token_moves_when_its_path_is_clear() -> None:
    ...
    with (
        patch("proof_video.animation.scene_helpers.Transform", return_value="crossing") as transform,
        patch("proof_video.animation.scene_helpers.FadeOut", return_value="fade-out"),
        patch("proof_video.animation.scene_helpers.FadeIn", return_value="fade-in"),
        patch("proof_video.animation.scene_helpers.AnimationGroup", side_effect=parallel),
        patch("proof_video.animation.scene_helpers.Succession", side_effect=sequence),
...
def test_crossing_tokens_preserve_every_mapped_object() -> None:
    ...
    with (
        patch("proof_video.animation.scene_helpers.Transform", return_value="mapped") as transform,
        patch("proof_video.animation.scene_helpers.FadeOut", side_effect=lambda token: ("out", token)),
        patch("proof_video.animation.scene_helpers.FadeIn", side_effect=lambda token: ("in", token)),
        patch("proof_video.animation.scene_helpers.AnimationGroup", side_effect=parallel),
        patch("proof_video.animation.scene_helpers.Succession", side_effect=sequence),
    ):
        animations = _mapped_row_animations(source, target, maps)
...
def test_fallback_preserves_only_same_text_with_same_full_bounds() -> None:
    ...
    with (
        patch("proof_video.animation.scene_helpers.Transform", return_value="stable") as transform,
        patch("proof_video.animation.scene_helpers.FadeOut", return_value="fade-out") as fade_out,
        patch("proof_video.animation.scene_helpers.FadeIn", return_value="fade-in") as fade_in,
        patch("proof_video.animation.scene_helpers.AnimationGroup", side_effect=parallel),
        patch("proof_video.animation.scene_helpers.Succession", side_effect=sequence),
...
def test_semantic_edges_are_preferred_to_conflicting_character_maps() -> None:
    ...
    with (
        patch("proof_video.animation.scene_helpers.Transform", side_effect=lambda a, b: (a, b)),
        patch("proof_video.animation.scene_helpers.FadeOut"),
        patch("proof_video.animation.scene_helpers.FadeIn"),
        patch("proof_video.animation.scene_helpers.AnimationGroup", side_effect=parallel),
        patch("proof_video.animation.scene_helpers.Succession", side_effect=sequence),
...
def test_certified_declaration_colon_is_not_reintroduced_as_a_new_symbol() -> None:
    ...
    transition = SemanticTransition(
        source=SemanticExpression((
            SemanticExpressionNode("name", kind="declaration", latex_spans=(SemanticSpan(0, 1),)),
            SemanticExpressionNode("colon", kind="declaration-punctuation", latex_spans=(SemanticSpan(2, 3),)),
            SemanticExpressionNode("type", kind="const", latex_spans=(SemanticSpan(4, 14),)),
        )),
        target=SemanticExpression((
            SemanticExpressionNode("name2", kind="declaration", latex_spans=(SemanticSpan(0, 1),)),
            SemanticExpressionNode("colon2", kind="declaration-punctuation", latex_spans=(SemanticSpan(2, 3),)),
            SemanticExpressionNode("type2", kind="const", latex_spans=(SemanticSpan(4, 14),)),
        )),
        edges=(
            SemanticTransitionEdge("name", "name2", "verified-binder-introduction", 1.0),
            SemanticTransitionEdge("colon", "colon2", "verified-binder-introduction", 1.0),
            SemanticTransitionEdge("type", "type2", "verified-binder-introduction", 1.0),
        ),
    )
...
def test_verified_atomic_rewrite_morphs_different_glyphs() -> None:
    transition = SemanticTransition(
        ...
        edges=(
            SemanticTransitionEdge(
                "old-f", "new-g", "verified-rewrite-position", 0.9
            ),
        ),
    )

    assert _semantic_token_pairs(
        ((0, 1),), ("f",), ((0, 1),), ("g",), transition
    ) == [(0, 0)]
...
def test_uncertified_leaf_identity_and_defeq_do_not_move_equal_glyphs() -> None:
    transition = SemanticTransition(
        source=SemanticExpression((
            SemanticExpressionNode(
                "source-leaf", kind="fvar", path=(0, 0),
                latex_spans=(SemanticSpan(0, 1),),
            ),
            SemanticExpressionNode(
                "source-parent", kind="app", path=(0,),
                latex_spans=(SemanticSpan(0, 1),),
            ),
        )),
        target=SemanticExpression((
            SemanticExpressionNode(
                "target-leaf", kind="fvar", path=(0, 0),
                latex_spans=(SemanticSpan(0, 1),),
            ),
            SemanticExpressionNode(
                "target-parent", kind="app", path=(1,),
                latex_spans=(SemanticSpan(2, 3),),
            ),
        )),
        edges=(
            SemanticTransitionEdge(
                "source-leaf", "target-leaf", "same-fvar", 1.0
            ),
            SemanticTransitionEdge(
...
def test_exact_composite_beats_conflicting_equal_leaf_symbols() -> None:
    ...
    transition = SemanticTransition(
        ...
        edges=(
            ...
            SemanticTransitionEdge(
                "old-f", "new-f-wrong", "same-identity", 1.0
            ),
        ),
    )
...
def test_infix_shell_does_not_claim_function_head_from_left_operand() -> None:
    ...
    transition = SemanticTransition(
        source=SemanticExpression((
            SemanticExpressionNode(
                "old-relation", kind="app", path=("0",),
                latex_spans=(SemanticSpan(0, 6),),
            ),
            SemanticExpressionNode(
                "old-left", kind="app", parent_id="old-relation", path=("0", "0"),
                latex_spans=(SemanticSpan(0, 3),),
            ),
            SemanticExpressionNode(
                "old-argument", kind="bvar", parent_id="old-left",
                path=("0", "0", "1"), latex_spans=(SemanticSpan(2, 3),),
            ),
            SemanticExpressionNode(
                "old-right", kind="bvar", parent_id="old-relation", path=("0", "1"),
                latex_spans=(SemanticSpan(5, 6),),
            ),
        )),
        target=SemanticExpression((
            SemanticExpressionNode(
                "new-relation", kind="app", path=("0",),
                latex_spans=(SemanticSpan(0, 12),),
            ),
            SemanticExpressionNode(
                "new-left", kind="app", parent_id="new-relation", path=("0", "0"),
                latex_spans=(SemanticSpan(0, 6),),
            ),
            SemanticExpressionNode(
                "new-argument", kind="app", parent_id="new-left",
                path=("0", "0", "1"), latex_spans=(SemanticSpan(2, 6),),
            ),
            SemanticExpressionNode(
                "new-right", kind="app", parent_id="new-relation", path=("0", "1"),
...
def test_write_speed_controls_glyph_reveal_time_and_rows_are_sequential() -> None:
    ...
    with (
        patch("proof_video.scene._glyph_reveal", side_effect=[first_reveal, second_reveal]),
...
def test_glyph_reveal_uses_true_stroke_writes_in_sequence() -> None:
    ...
    with (
        ...
        patch("proof_video.animation.scene_helpers.Write", side_effect=lambda glyph: ("write", glyph)) as write,
        patch("proof_video.animation.scene_helpers.Succession", return_value="succession") as succession,
    ):
