...
def ensure_extractor_executable(root: Path) -> Path:
    ...
    if result.returncode == 0:
        ...
        os.name == "nt"
        and _canonical_output_is_locked(details)
    ):
        print(
...
def _lean_runtime_path(root_text: str, inherited: str) -> str:
    ...
    if inherited:
        search_paths.extend(Path(value) for value in inherited.split(os.pathsep) if value)
    return os.pathsep.join(
        str(path.resolve()) for path in dict.fromkeys(search_paths)
    )
