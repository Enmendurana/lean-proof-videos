...
-- proof-video: theorem erdos_problem_38
...
lemma bad_min_count_lt (m : ℕ) (_hm : 20 ≤ m) :
  ...
  have h_bad_count : ∀ j : Fin (shiftL m), (Finset.univ.filter (fun t : Fin (2 ^ m) => (t : ℕ) + 1 ≤ 2 ^ m / (2 * shiftL m + 2))).card ≤ 2 ^ m / (2 * shiftL m + 2) := by
    ...
    simpa only [Nat.lt_iff_add_one_le] using
      (show (Finset.univ.filter (fun t : Fin (2 ^ m) =>
        (t : ℕ) < 2 ^ m / (2 * shiftL m + 2))).card ≤
          2 ^ m / (2 * shiftL m + 2) from by
  ...
  have h_union_bound : (Finset.univ.filter (fun ω : Fin (shiftL m) → Fin (2 ^ m) => ∃ j, (ω j : ℕ) + 1 ≤ 2 ^ m / (2 * shiftL m + 2))).card ≤ shiftL m * (2 ^ m / (2 * shiftL m + 2)) * (2 ^ m) ^ (shiftL m - 1) := by
    have h_union_bound : (Finset.univ.filter (fun ω : Fin (shiftL m) → Fin (2 ^ m) => ∃ j, (ω j : ℕ) + 1 ≤ 2 ^ m / (2 * shiftL m + 2))).card ≤ ∑ j : Fin (shiftL m), (Finset.univ.filter (fun t : Fin (2 ^ m) => (t : ℕ) + 1 ≤ 2 ^ m / (2 * shiftL m + 2))).card * (2 ^ m) ^ (shiftL m - 1) := by
      have h_union_bound : ∀ j : Fin (shiftL m), (Finset.univ.filter (fun ω : Fin (shiftL m) → Fin (2 ^ m) => (ω j : ℕ) + 1 ≤ 2 ^ m / (2 * shiftL m + 2))).card ≤ (Finset.univ.filter (fun t : Fin (2 ^ m) => (t : ℕ) + 1 ≤ 2 ^ m / (2 * shiftL m + 2))).card * (2 ^ m) ^ (shiftL m - 1) := by
        intro j
        let bad := Finset.univ.filter (fun t : Fin (2 ^ m) =>
          (t : ℕ) + 1 ≤ 2 ^ m / (2 * shiftL m + 2))
        have h_fiber (t : Fin (2 ^ m)) :
            (Finset.univ.filter (fun ω : Fin (shiftL m) → Fin (2 ^ m) =>
              ω j = t)).card = (2 ^ m) ^ (shiftL m - 1) := by
          simpa using Fintype.card_filter_piFinset_const_eq_of_mem
            (Finset.univ : Finset (Fin (2 ^ m))) j (Finset.mem_univ t)
        rw [show Finset.univ.filter (fun ω : Fin (shiftL m) → Fin (2 ^ m) =>
            (ω j : ℕ) + 1 ≤ 2 ^ m / (2 * shiftL m + 2)) =
            bad.biUnion (fun t => Finset.univ.filter fun ω => ω j = t) by
          ext ω
          simp [bad]]
        calc
          _ ≤ ∑ t ∈ bad,
              (Finset.univ.filter (fun ω : Fin (shiftL m) → Fin (2 ^ m) =>
                ω j = t)).card := Finset.card_biUnion_le
          _ = bad.card * (2 ^ m) ^ (shiftL m - 1) := by
            simp_rw [h_fiber]
            simp
      refine' le_trans _ ( Finset.sum_le_sum fun j _ => h_union_bound j )
      rw [ show ( Finset.univ.filter fun ω : Fin ( shiftL m ) → Fin ( 2 ^ m ) => ∃ j : Fin ( shiftL m ), ( ω j : ℕ ) + 1 ≤ 2 ^ m / ( 2 * shiftL m + 2 ) ) = Finset.biUnion Finset.univ fun j => Finset.univ.filter fun ω : Fin ( shiftL m ) → Fin ( 2 ^ m ) => ( ω j : ℕ ) + 1 ≤ 2 ^ m / ( 2 * shiftL m + 2 ) by ext; simp ]
...
-- proof-video: module-end per_m_good_shifts_hard
...
-- proof-video: module-end shift_approx_exists
...
-- proof-video: module-end constructB_not_basis
...
-- proof-video: module-end density_increment
